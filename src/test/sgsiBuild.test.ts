...
describe('buildSgsiResponse', () => {
  const rows: SgsiRawItem[] = [
    // 010 — mudanças
    item('010', 11, { Status: 'Concluído', Ambiente: 'Produção', Risco: 'Baixo', 'Atualizações bem sucedidas': 'Sim', 'Número do chamado': 'MUD-0011' }, '2026-05-01T08:00:00Z'),
    item('010', 12, { Status: 'Aguardando Aprovação TI', Ambiente: 'Homologação', Risco: 'Alto', 'Atualizações bem sucedidas': 'Não' }, '2026-06-01T08:00:00Z'),
    item('010', 13, { Status: 'Aguardando Gestor', Ambiente: 'Produção' }, '2026-06-05T08:00:00Z'),
    ...
    // 012 — riscos
    item('012', 31, { 'Status solicitação': 'Tratado', 'CID afetado': 'Disponibilidade', 'O plano de tratamento de risco foi eficaz?': 'Sim' }, '2026-06-09T00:00:00Z'),
    item('012', 32, { 'Status solicitação': 'Em tratamento', 'CID afetado': 'Integridade' }, '2026-04-01T00:00:00Z'),
    // 018 — NC
  ...
  it('separa os blocos por lista e calcula os KPIs de mudanças', () => {
    expect(r.totalItens).toBe(12);
    expect(r.mudancas.total).toBe(3);
    expect(r.mudancas.concluidos).toBe(1);
    expect(r.mudancas.pendentes).toBe(2);
  ...
  it('riscos: abertos = não tratados/encerrados', () => {
    expect(r.riscos.total).toBe(2);
    expect(r.riscos.abertos).toBe(1);
