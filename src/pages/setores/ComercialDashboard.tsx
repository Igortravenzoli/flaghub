  ...
  return (
    <SectorLayout title="Dashboard Único Q1 2026" subtitle="Comercial — Visão Consolidada" lastUpdate="19/02/2026 09:30">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <BlockCard icon={TrendingUp} title="Topo Executivo">
          <div className="space-y-3">
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">R$ {(d.executivo.receitaQ1 / 1e6).toFixed(1)}M</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Meta atingida</p>
              <div className="flex items-center gap-2">
                <Progress value={d.executivo.metaAtingida} className="flex-1" />
                <span className="text-sm font-semibold text-foreground">{d.executivo.metaAtingida}%</span>
              </div>
            </div>
            <div>
        ...
        {/* Pipeline */}
        <BlockCard icon={BarChart3} title="Bloco Pipeline">
          <div className="space-y-2">
            {d.pipeline.map((p) => (
              <div key={p.etapa} className="flex items-center justify-between">
        ...
        {/* Clientes */}
        <BlockCard icon={Users} title="Bloco Clientes">
          <div className="grid grid-cols-2 gap-4 mb-3">
            <div className="text-center rounded-lg bg-[hsl(var(--success))]/10 p-3">
              <p className="text-2xl font-bold text-[hsl(var(--success))]">+{d.clientes.novos}</p>
              <p className="text-xs text-muted-foreground">Novos</p>
            </div>
            <div className="text-center rounded-lg bg-[hsl(var(--critical))]/10 p-3">
              <p className="text-2xl font-bold text-[hsl(var(--critical))]">-{d.clientes.perdidos}</p>
        ...
        {/* Satisfação */}
        <BlockCard icon={Heart} title="Bloco Satisfação">
          <div className="text-center mb-4">
            <p className="text-4xl font-bold text-primary">{d.satisfacao.nps}</p>
            <p className="text-sm text-muted-foreground">NPS Score</p>
          </div>
          <Card className="p-3 bg-[hsl(var(--critical))]/10 border-[hsl(var(--critical))]/20">
