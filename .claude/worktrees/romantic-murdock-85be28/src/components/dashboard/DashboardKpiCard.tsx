...
const KPI_TOOLTIP_MAP: Record<string, { formula: string; description: string }> = {
  ...
  'Fila QA (Atual)': { formula: 'COUNT(state IN Em Teste, Aguardando Deploy)', description: 'Fila atual oficial da Qualidade, incluindo itens herdados de sprints passadas.' },
  ...
  'Em Teste': { formula: 'COUNT(state = Em Teste)', description: 'Itens efetivamente em execução de testes no escopo filtrado.' },
  'Aguardando Deploy': { formula: 'COUNT(state = Aguardando Deploy)', description: 'Itens já testados e aguardando janela de deploy.' },
