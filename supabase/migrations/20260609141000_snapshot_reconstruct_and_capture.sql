-- ============================================================================
-- Migration: 20260609141000_snapshot_reconstruct_and_capture.sql
-- Reconstrução as-of fim de sprint (Closed By) + captura com procedência
--
--   1. rpc_reconstruct_sprint_snapshot  — estado real em 23:59 do fim da sprint,
--      via state_history; calcula qa_concluidos (Closed By autorizado).
--   2. rpc_capture_sprint_snapshot      — passa a gravar qa_concluidos +
--      snapshot_source='estado_atual' + as_of_datetime=now().
--   3. rpc_backfill_reconstruct_closed_sprints(year) — driver p/ todas as
--      sprints fechadas; cron job 51 passa a usá-lo.
-- ============================================================================

-- ── 1. Reconstrução as-of fim de sprint ──────────────────────────────────────
DROP FUNCTION IF EXISTS public.rpc_reconstruct_sprint_snapshot(text, timestamptz);
...
BEGIN
  ...
  SELECT
    COUNT(*),
    COUNT(*) FILTER (WHERE NOT (tags_text ILIKE '%Retorno de QA%' OR (wtype='Bug' AND tags_text NOT ILIKE '%Retorno de QA%') OR ((tags_text ILIKE '%Avião%' OR tags_text ILIKE '%Aviao%') AND tags_text NOT ILIKE '%Retorno de QA%'))),
    COUNT(*) FILTER (WHERE (tags_text ILIKE '%Retorno de QA%' OR (wtype='Bug' AND tags_text NOT ILIKE '%Retorno de QA%') OR ((tags_text ILIKE '%Avião%' OR tags_text ILIKE '%Aviao%') AND tags_text NOT ILIKE '%Retorno de QA%'))),
...
GRANT EXECUTE ON FUNCTION public.rpc_reconstruct_sprint_snapshot(text, timestamptz) TO authenticated, service_role;

COMMENT ON FUNCTION public.rpc_reconstruct_sprint_snapshot(text, timestamptz) IS
  'Reconstrói o snapshot da sprint no estado real de fim de sprint (23:59 BRT por padrão) '
  'usando state_history; calcula qa_concluidos (Closed By autorizado). Substitui snapshots anteriores da sprint.';

-- ── 2. Captura ao vivo: qa_concluidos + procedência ──────────────────────────
...
-- ── 3. Driver de backfill por reconstrução + cron ────────────────────────────
DROP FUNCTION IF EXISTS public.rpc_backfill_reconstruct_closed_sprints(int);
...
BEGIN
  ...
  LOOP
    ...
    sprint_code := v_sprint; status := 'reconstructed';
...
GRANT EXECUTE ON FUNCTION public.rpc_backfill_reconstruct_closed_sprints(int) TO authenticated, service_role;

-- Aponta o cron de fim de sprint para a reconstrução
DO $$ BEGIN
  PERFORM cron.alter_job(
    51,
    command => 'SELECT public.rpc_backfill_reconstruct_closed_sprints(EXTRACT(YEAR FROM NOW())::int);'
  );
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'cron.alter_job falhou (%). Atualize o job 51 manualmente.', SQLERRM;
END $$;
