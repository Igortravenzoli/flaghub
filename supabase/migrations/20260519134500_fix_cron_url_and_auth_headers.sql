-- Corrige URLs de cron jobs que ficaram com caminho duplicado e garante header Authorization.
...
DECLARE
  ...
  v_job record;
BEGIN
  SELECT substring(j.command from '(https://[^/]+\.supabase\.co)') INTO v_base_url
  FROM cron.job j
